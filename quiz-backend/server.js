...
// Get questions (with remaining time)
app.get("/questions", (req, res) => {
    ...
    const now = Date.now();
    if (now > latestSession.endTime) {
        return res.status(403).json({ error: "Quiz ended" });
    }
    ...
    res.json({
        remainingTimeMs: latestSession.endTime - now,
        questions
    });
});
